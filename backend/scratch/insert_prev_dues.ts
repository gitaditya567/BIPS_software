...
const studentsData = [
  ...
  { name: 'Shagun Gupta', contact: '8787016061', father: 'Sandeep Gupta', address: 'Bhadesua', class: 'CLASS 7', preBal: 900 },
...
async function insertNotFound() {
  ...
  for (const item of studentsData) {
    ...
    if (students.length === 0) {
      ...
      const className = classMap[item.class] || item.class;
      let classObj = await prisma.class.findFirst({
        where: { name: className }
      });
      if (!classObj) {
        classObj = await prisma.class.create({ data: { name: className } });
      }

      const email = `${item.name.replace(/\\s/g, '').toLowerCase()}${Math.floor(Math.random() * 1000)}@bips.local`;
      ...
      const nextNo = await prisma.studentProfile.count();
      const admissionNo = `BIPS/26/${String(nextNo + 500).padStart(3, '0')}`; // padding
      const studentId = `STU-2026-${String(Math.floor(Math.random() * 10000)).padStart(4, '0')}-${count}`;
      ...
      await prisma.studentProfile.create({
        data: {
          ...
          previousSessionDue: item.preBal,
          class: { connect: { id: classObj.id } }
        }
      });
  ...
  console.log(`Inserted ${count} new students.`);
}

insertNotFound()
  .then(() => console.log('Done insertion'))
