...
router.post('/students', async (req, res) => {
    try {
        ...
        const studentEmail = email || `${finalAdmissionNo.replace(/[^a-zA-Z0-9]/g, '_').toLowerCase()}@bips.local`;
...
router.put('/students/:id', async (req, res) => {
    try {
        ...
        const updatedStudent = await prisma.studentProfile.update({
            where: { id },
            data: {
                admissionNo: admissionNo || profile.admissionNo,
...
router.get('/dashboard/stats', async (req, res) => {
    try {
        ...
        const pendingFees = (pendingPayments as any)._sum?.amountPaid || 0;
