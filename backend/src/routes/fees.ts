...
function parsePaymentBreakdown(feeHead: string | null, month: string | null, amountPaid: number, discount: number, feeHeadsList: any[]): ParsedItem[] {
    ...
    if (feeHead.includes('==>') || feeHead.includes('||') || feeHead.includes(':')) {
        ...
        // Use proportional allocation for partial payments to distribute lump-sum deposits fairly
        const isPartial = totalItemsAmount > 0 && totalAmount < totalItemsAmount;
        let remainingToAllocate = totalAmount;
        tempItems.forEach((tempItem, idx) => {
            const { name, amt } = tempItem;
            let allocatedAmt = 0;
            if (isPartial) {
                if (idx === tempItems.length - 1) {
                    allocatedAmt = remainingToAllocate;
                } else {
                    allocatedAmt = Math.round((amt / totalItemsAmount) * totalAmount);
                    allocatedAmt = Math.min(remainingToAllocate, allocatedAmt);
                }
            } else {
            ...
            const isPreviousDues = nameLower.startsWith('previous dues') || nameLower.startsWith('previous due') || nameLower.includes('previous due');
            const isTransport = !isPreviousDues && (nameLower.includes('transport') || nameLower.includes('bus'));
            ...
            const isMonthly = !isPreviousDues && (isTransport || (headObj ? headObj.type === 'Monthly' : false));
            const isOneTime = !isPreviousDues && (headObj ? (headObj.type === 'One-time' || headObj.type === 'Annual' || headObj.type === 'Other') : false);
...
export async function getStudentFeeLedger(studentId: string) {
    ...
    const initialPrevDue = student.previousSessionDue || 0;
    let remainingInitialPrevDue = initialPrevDue;

    const pendingDuesReceipts: { receipt: any; amount: number }[] = [];

    payments.forEach(p => {
        const parsedItems = parsePaymentBreakdown(p.feeHead, p.month, p.amountPaid || 0, p.discount || 0, feeHeads);
        parsedItems.forEach(item => {
            if (item.isPreviousDues) {
                pendingDuesReceipts.push({ receipt: p, amount: item.amount });
    ...
    // Allocate previous dues receipts chronologically to settle outstanding obligations
    pendingDuesReceipts.forEach(({ receipt: p, amount }) => {
        let unallocatedDues = amount;

        // 1. Settle initial previousSessionDue first
        if (remainingInitialPrevDue > 0) {
            const toInitial = Math.min(remainingInitialPrevDue, unallocatedDues);
            actualPrevDuesPaid += toInitial;
            remainingInitialPrevDue -= toInitial;
            unallocatedDues -= toInitial;
        }
        if (unallocatedDues <= 0) return;

        // Helper to calculate true unpaid shortfall for a given month m
        const getMonthShortfall = (m: string) => {
            let headShortfall = 0;
            monthlyExpectedBreakdown.forEach(h => {
                const direct = monthlyPaidForHeadAndMonth[h.name]?.[m] || 0;
                if (direct < h.amount) {
                    headShortfall += (h.amount - direct);
                }
            });
            const transExp = transportMonthlyFare > 0 ? transportMonthlyFare : (transportPaidForMonth[m] || 0);
            const transDirect = transportPaidForMonth[m] || 0;
            const transShortfall = Math.max(0, transExp - transDirect);
            const alreadyAllocatedGeneric = genericMonthlyPaidForMonth[m] || 0;
        ...
        oneTimeExpectedBreakdown.forEach(ot => {
            ...
            const pending = Math.max(0, ot.amount - currentPaid);
            if (pending > 0) {
                const wasBilled = payments.some(rec => rec.feeHead && rec.feeHead.toLowerCase().includes(ot.name.toLowerCase()));
                if (wasBilled) {
                    const toOt = Math.min(pending, unallocatedDues);
                    oneTimePaidForHead[ot.name] = currentPaid + toOt;
                    actualOneTimePaid += toOt;
                    unallocatedDues -= toOt;
                }
            }
        });
        if (unallocatedDues <= 0) return;
        ...
        for (const m of allMonths) {
            if (unallocatedDues <= 0) break;
            const shortfall = getMonthShortfall(m);
            if (shortfall > 0) {
                const toM = Math.min(shortfall, unallocatedDues);
                genericMonthlyPaidForMonth[m] = (genericMonthlyPaidForMonth[m] || 0) + toM;
                actualMonthlyPaid += toM;
                unallocatedDues -= toM;
            }
        }

        if (unallocatedDues > 0) {
            actualMonthlyPaid += unallocatedDues;
            genericMonthlyPaidForMonth['UNSPECIFIED'] = (genericMonthlyPaidForMonth['UNSPECIFIED'] || 0) + unallocatedDues;
        }
    });
    ...
    let poolGeneric = genericMonthlyPaidForMonth['UNSPECIFIED'] || 0;
    ...
    allMonths.forEach(m => {
        ...
        const hasDirectPayment = (monthWisePaid[m] || 0) > 0 || (transportPaidForMonth[m] || 0) > 0;
        const canUsePoolGeneric = hasDirectPayment || currentMonthGenericMoney > 0;
        ...
        monthlyExpectedBreakdown.forEach(head => {
            ...
            if (allocated < expected && (headRolloverPool[head.name] || 0) > 0 && canUsePoolGeneric) {
            ...
            if (allocated < expected && poolGeneric > 0 && canUsePoolGeneric) {
                const needed = expected - allocated;
                const cover = Math.min(needed, poolGeneric);
                allocated += cover;
                poolGeneric -= cover;
            }
        ...
        // Cross-head rollover settlement for month m:
        // Any credit or excess money from earlier months settles shortfalls in the current month before rolling forward
        headsBreakdown.forEach(h => {
            if (h.paid < h.expected && canUsePoolGeneric) {
                for (const otherHead of monthlyExpectedBreakdown) {
                    if ((headRolloverPool[otherHead.name] || 0) > 0) {
                        const needed = h.expected - h.paid;
                        const cover = Math.min(needed, headRolloverPool[otherHead.name]);
                        h.paid += cover;
                        h.pending = Math.max(0, h.expected - h.paid);
                        headRolloverPool[otherHead.name] -= cover;
                        monthPaidTotal += cover;
                        if (isElapsed) {
                            monthlyPending -= cover;
                        }
                        if (h.paid >= h.expected) break;
                    }
                }
                if (h.paid < h.expected && rolloverTransportPool > 0) {
                    const needed = h.expected - h.paid;
                    const cover = Math.min(needed, rolloverTransportPool);
                    h.paid += cover;
                    h.pending = Math.max(0, h.expected - h.paid);
                    rolloverTransportPool -= cover;
                    monthPaidTotal += cover;
                    if (isElapsed) {
                        monthlyPending -= cover;
                    }
        ...
        if (transportShortfall > 0 && rolloverTransportPool > 0 && canUsePoolGeneric) {
        ...
        if (transportShortfall > 0 && canUsePoolGeneric) {
            for (const otherHead of monthlyExpectedBreakdown) {
                if ((headRolloverPool[otherHead.name] || 0) > 0) {
                    const cover = Math.min(transportShortfall, headRolloverPool[otherHead.name]);
                    transportAllocated += cover;
                    headRolloverPool[otherHead.name] -= cover;
                    transportShortfall -= cover;
                    if (transportShortfall <= 0) break;
                }
            }
        }

        if (transportShortfall > 0 && poolGeneric > 0 && canUsePoolGeneric) {
            const extraTransport = Math.min(transportShortfall, Math.round(poolGeneric));
            transportAllocated += extraTransport;
            poolGeneric = Math.max(0, poolGeneric - extraTransport);
            transportShortfall = Math.max(0, transportExpected - transportAllocated);
        }

        // If this month had extra generic money left over, roll it forward into poolGeneric for subsequent months
        if (currentMonthGenericMoney > 0) {
            poolGeneric += currentMonthGenericMoney;
            currentMonthGenericMoney = 0;
        }
    ...
    return {
        ...
        summary: {
            ...
            totalPaidAllTime,
            netOutstanding,
            advanceBalance: Math.round(poolGeneric + rolloverTransportPool + Object.values(headRolloverPool).reduce((s, v) => s + v, 0))
        },
        advanceBalance: Math.round(poolGeneric + rolloverTransportPool + Object.values(headRolloverPool).reduce((s, v) => s + v, 0)),
        oneTimeStatus,
        monthlyStatus,
...
router.get('/transport-due-list', async (req, res) => {
    try {
        ...
        const transportStudentIdsFromPayments = [...new Set(
            allPayments
                .filter(p => p.feeHead && (p.feeHead.toLowerCase().includes('transport') || p.feeHead.toLowerCase().includes('bus')))
                .map(p => p.studentId)
        )];
        ...
        if (requestedMonth && requestedMonth !== 'all' && allMonths.includes(requestedMonth)) {
            const idx = allMonths.indexOf(requestedMonth);
            elapsedMonths = allMonths.slice(0, idx + 1);
        }
        ...
        const dueList = students.map(student => {
            ...
            studentPayments.forEach(p => {
                if (!student.transportStopId && p.feeHead) {
                    const match = p.feeHead.match(/(?:Transport|Bus)\s*(?:\((.*?)\))?(?:\s*\(Yearly\))?:\s*(\d+)/i);
                    if (match) {
                        if (stopName === 'N/A') {
                            stopName = match[1] ? match[1].trim() : 'Custom Transport';
                        }
                        if (monthlyFare === 0) {
                            const isYearly = p.feeHead.toLowerCase().includes('yearly');
                            const mthsCount = p.month ? p.month.split(',').length : 1;
                            monthlyFare = isYearly ? Number(match[2]) / 12 : Number(match[2]) / mthsCount;
                        }
                    }
                }
            });
            ...
            studentPayments.forEach(p => {
                ...
                parsedItems.forEach(item => {
                    if (item.isPreviousDues) {
                        ...
                        actualTransportPaid += item.amount;
                        const mths = item.months.length > 0 ? item.months : (p.month ? p.month.split(',').map((m: string) => m.trim()) : []);
                        if (mths.length > 0) {
            ...
            pendingDuesReceipts.forEach(({ receipt: p, amount }) => {
                ...
                const targetMonth = (p.month && allMonths.includes(p.month)) ? p.month : (p.feeHead ? allMonths.find(m => p.feeHead!.includes(m)) : null);
                if (targetMonth) {
                    const monthExp = monthlyExpectedBreakdown.reduce((sum, h) => sum + h.amount, 0) + monthlyFare;
                    let directPaidForMonth = (genericMonthlyPaidForMonth[targetMonth] || 0) + (transportPaidForMonth[targetMonth] || 0);
                    monthlyExpectedBreakdown.forEach(h => {
                        directPaidForMonth += (monthlyPaidForHeadAndMonth[h.name]?.[targetMonth] || 0);
                    });
                    const monthShortfall = Math.max(0, monthExp - directPaidForMonth);
                    if (monthShortfall > 0) {
                        const toMonth = Math.min(monthShortfall, unallocatedDues);
                        genericMonthlyPaidForMonth[targetMonth] = (genericMonthlyPaidForMonth[targetMonth] || 0) + toMonth;
                        actualMonthlyPaid += toMonth;
                ...
                oneTimeExpectedBreakdown.forEach(ot => {
                    ...
                    const pending = Math.max(0, ot.amount - currentPaid);
                    if (pending > 0) {
                        const wasBilled = studentPayments.some(rec => rec.feeHead && rec.feeHead.toLowerCase().includes(ot.name.toLowerCase()));
                        if (wasBilled) {
                            const toOt = Math.min(pending, unallocatedDues);
                            oneTimePaidForHead[ot.name] = currentPaid + toOt;
                            actualOneTimePaid += toOt;
                ...
                for (const m of allMonths) {
                    if (unallocatedDues <= 0) break;
                    const monthExp = monthlyExpectedBreakdown.reduce((sum, h) => sum + h.amount, 0) + monthlyFare;
                    let directPaid = (genericMonthlyPaidForMonth[m] || 0) + (transportPaidForMonth[m] || 0);
                    monthlyExpectedBreakdown.forEach(h => {
                        directPaid += (monthlyPaidForHeadAndMonth[h.name]?.[m] || 0);
                    });
                    const shortfall = Math.max(0, monthExp - directPaid);
                    if (shortfall > 0) {
                        const toM = Math.min(shortfall, unallocatedDues);
                        genericMonthlyPaidForMonth[m] = (genericMonthlyPaidForMonth[m] || 0) + toM;
                        actualMonthlyPaid += toM;
            ...
            allMonths.forEach(m => {
                ...
                monthlyExpectedBreakdown.forEach(head => {
                    ...
                    if (allocated < expected && poolGeneric > 0) {
                        const needed = expected - allocated;
                        const cover = Math.min(needed, poolGeneric);
                        allocated += cover;
                        poolGeneric -= cover;
                    ...
                    monthlyPaidForHeadAndMonth[head.name][m] = allocated;
                });

                // Cross-head rollover settlement for month m
                monthlyExpectedBreakdown.forEach(h => {
                    const expected = h.amount;
                    let allocated = monthlyPaidForHeadAndMonth[h.name]?.[m] || 0;
                    if (allocated < expected) {
                        for (const otherHead of monthlyExpectedBreakdown) {
                            if ((headRolloverPool[otherHead.name] || 0) > 0) {
                                ...
                                allocated += cover;
                                headRolloverPool[otherHead.name] -= cover;
                                monthlyPaidForHeadAndMonth[h.name][m] = allocated;
                                if (allocated >= expected) break;
                            }
                        }
                    }
                });
                ...
                if (transportShortfall > 0 && poolGeneric > 0) {
                    const extraTransport = Math.min(transportShortfall, Math.round(poolGeneric));
                    transportAllocated += extraTransport;
                    poolGeneric = Math.max(0, poolGeneric - extraTransport);
                }
                ...
                transportAllocated = Math.round(transportAllocated);
                monthWisePaid[m] = transportAllocated;
                totalAllocatedTransportPaid += transportAllocated;
            });

            // Calculate fully paid months where collected fare is >= monthlyFare
            const calculatedPaidMonths = allMonths.filter(m => {
                if (monthlyFare <= 0) return (monthWisePaid[m] || 0) > 0;
                return (monthWisePaid[m] || 0) >= monthlyFare;
            });

            const expectedTotal = monthlyFare * elapsedMonths.length;
            const pending = Math.max(0, expectedTotal - totalAllocatedTransportPaid);
            const advancePaid = Math.max(0, totalAllocatedTransportPaid - expectedTotal);

            if (monthlyFare === 0 && totalAllocatedTransportPaid === 0) return null;
            ...
            return {
                ...
                totalPaid: Math.round(totalAllocatedTransportPaid),
...
router.post('/collect', async (req, res) => {
    try {
        const {
            ...
            feeHead, paymentMode, month, year, submittedBy, remark, requiresApproval
        } = req.body;
        ...
        const isPending = Boolean(Number(discount) > 0 && (requiresApproval === true || requiresApproval === 'true'));
        ...
        const studentObj = await prisma.studentProfile.findUnique({
            where: { id: studentId },
            include: { user: true, class: true }
        });
        const studentNameDb = req.body.studentName || studentObj?.user?.name || 'Student';
        const admissionNoDb = req.body.admissionNo || studentObj?.admissionNo || '';
        const classNameDb = req.body.className || studentObj?.class?.name || '';
        ...
        const feePayment = await prisma.feePayment.create({
            data: {
                studentId,
                studentName: studentNameDb,
                admissionNo: admissionNoDb,
...
router.get('/public/student-dues', async (req: express.Request, res: express.Response): Promise<any> => {
    try {
        ...
        // 3. Fetch Approved & Pending Payments for receipt history & verification tracking
        const allPayments = await prisma.feePayment.findMany({
            where: {
        ...
        const approvedReceipts = allPayments
        ...
        const pendingTransactions = allPayments
            ...
            .map(p => ({
                id: p.id,
                txnid: p.txnid || 'PENDING-TXN',
                date: p.paymentDate ? new Date(p.paymentDate).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' }) : new Date().toLocaleDateString('en-GB'),
        ...
        const feeHeadsAll = await prisma.feeHead.findMany();
        const feeStructure: any = student.class?.feeStructure || {};
        let monthlyDiscount = 0;
        let discountType = 'None';
        let discountDescription = 'No concession applied';

        if (student.isRT) {
            // RTE students: all monthly fees are exempt
            feeHeadsAll.forEach((h: any) => {
                if (h.type === 'Monthly') {
                    const amt = feeStructure[h.name] || 0;
                    if (amt > 0) monthlyDiscount += amt;
                }
            });
            // Also transport if any
            if (student.transportStop?.busFare) {
                monthlyDiscount += student.transportStop.busFare;
            }
            discountType = 'RTE';
            discountDescription = 'RTE Student — 100% monthly fee exemption';
        } else if (student.isThirdChild) {
            // Third child: exempt from all monthly fees EXCEPT Computer Class Fee (for Class 1-8/Pre-primary)
            const className = student.class?.name || '';
            const isPrePrimOr1to8 = (() => {
                const n = className.toLowerCase();
                if (n.includes('nursery') || n.includes('lkg') || n.includes('ukg') || n.includes('kindergarten') || n.includes('play')) return true;
                const m = n.match(/class\s+(\d+)/);
                if (m) { const num = parseInt(m[1]); return num >= 1 && num <= 8; }
                return false;
            })();
            feeHeadsAll.forEach((h: any) => {
                if (h.type === 'Monthly') {
                    const isComputerFee = h.name.toLowerCase().includes('computer class fee');
                    const isExempt = !(isComputerFee && isPrePrimOr1to8);
                    if (isExempt) {
                        const amt = feeStructure[h.name] || 0;
                        if (amt > 0) monthlyDiscount += amt;
                    }
            ...
            discountType = 'ThirdChild';
            discountDescription = 'Third Child — Tuition free, only Computer & Transport fee applicable';
        }

        const sessionDiscount = monthlyDiscount * 12;
        ...
        res.json({
            ...
            monthlyDues: ledger.monthlyStatus.map((m: any) => {
                ...
                return {
                    ...
                    isPaid: isFullyPaid,
                    heads: m.heads || [],
                    isAdvance: false // will be computed on frontend based on month index vs current
                };
            }),
            approvedReceipts,
            pendingTransactions
    ...
    try {
        ...
        const dueList = students.map(student => {
            ...
            allMonths.forEach(m => {
                ...
                // Cross-head settlement for month m
                headsBreakdown.forEach(h => {
                    if (h.paid < h.expected && canUsePoolGeneric) {
                        for (const otherHead of monthlyExpectedBreakdown) {
                            if ((headRolloverPool[otherHead.name] || 0) > 0) {
            ...
            const advanceBalance = Math.round(poolGeneric + rolloverTransportPool + Object.values(headRolloverPool).reduce((s, v) => s + v, 0));
            ...
            const rolledOverMonthWiseGeneralPaid: Record<string, number> = {};
            const rolledOverMonthWiseTransportPaid: Record<string, number> = {};
            const rolledOverMonthWiseTotalPaid: Record<string, number> = {};

            monthlyStatus.forEach(ms => {
                let genPaid = 0;
                let trPaid = 0;
                (ms.heads || []).forEach((h: any) => {
                ...
                rolledOverMonthWiseGeneralPaid[ms.month] = genPaid;
                rolledOverMonthWiseTransportPaid[ms.month] = trPaid;
                rolledOverMonthWiseTotalPaid[ms.month] = ms.paid;
            });
            ...
            return {
                ...
                expectedOneTime: expectedOneTimeTotal,
                oneTimeBreakdown,
                monthWisePaid: rolledOverMonthWiseGeneralPaid,
                monthWiseTransportPaid: rolledOverMonthWiseTransportPaid,
                monthWiseTotalPaid: rolledOverMonthWiseTotalPaid,
                previousSessionDue,
                prevDuePending,
                monthlyFeeAmount: monthlyFeeAmountValue,
    ...
    try {
        ...
        if (authHeader && authHeader.startsWith('Bearer ')) {
            ...
            try {
                ...
                const role = decoded?.role ? String(decoded.role).toUpperCase() : '';
                const allowedRoles = ['ADMIN', 'SUPERADMIN', 'SUPER_ADMIN', 'SUPERADMINISTRATOR'];
                if (role && !allowedRoles.includes(role)) {
                    return res.status(403).json({ error: 'Forbidden: Only Super Admin can delete fee receipts' });
                }
            } catch (jwtErr) {
                console.warn('JWT verification warning in delete fee receipt:', jwtErr);
            }
        }
...
router.post('/payu/initiate', async (req: express.Request, res: express.Response): Promise<any> => {
    try {
        ...
        const callerOrigin = req.body.clientOrigin || req.headers['origin'] || req.headers['referer'] || '';
        const originClean = typeof callerOrigin === 'string' ? callerOrigin.replace(/\/+$/, '') : '';
        const sourceFlag = req.body.udf4 || 'Admin';
        const encodedUdf4 = originClean ? `${sourceFlag}@@${originClean}::${feeHead || ''}` : `${sourceFlag}::${feeHead || ''}`;
...
const payuResponse = async (req: express.Request, res: express.Response): Promise<any> => {
    try {
        ...
        const udf4Raw = String(postData.udf4 || '');
        const isFromAdmin = udf4Raw.includes('Admin');
        const isFromPublic = udf4Raw.includes('Public') || udf4Raw.includes('FeeOnline');
        ...
        let frontendBase = '';
        if (udf4Raw.includes('@@')) {
            const candidate = udf4Raw.split('@@')[1]?.split('::')[0]?.trim();
            if (candidate && (candidate.startsWith('http://') || candidate.startsWith('https://'))) {
                frontendBase = candidate.replace(/\/+$/, '');
            }
        }
