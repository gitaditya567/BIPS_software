...
import { getStudentFeeLedger } from './fees';
...
router.get('/dashboard-stats/student/:studentId', async (req, res) => {
    try {
        ...
        // 2. Fee Dues (Real Net Outstanding Dues)
        let totalFeeDue = 0;
        try {
            const ledger = await getStudentFeeLedger(studentId);
            totalFeeDue = Math.max(0, Math.round(ledger.summary?.netOutstanding || 0));
        } catch {
            const pendingFees = await prisma.feePayment.findMany({ where: { studentId, status: 'PENDING' } });
            totalFeeDue = pendingFees.reduce((sum, fee) => sum + (fee.amountPaid || fee.totalFee || 0), 0);
        }
        ...
        res.json({
            stats: {
                ...
                feeDues: '₹' + totalFeeDue.toLocaleString('en-IN'),
