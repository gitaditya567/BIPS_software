...
import { Search, Clock, Download, Smartphone, WifiOff, Menu } from 'lucide-react';
    ...
    return (
        <div className="topbar-clock" style={{
            ...
            <div style={{ lineHeight: 1.2 }}>
                ...
                <p className="topbar-clock-date" style={{
                    margin: 0,
                    fontSize: '0.68rem',
                    fontWeight: '600',
                    color: '#4a90e2',
...
const Topbar: React.FC<TopbarProps> = ({ onOpenInstallModal, onMenuClick }) => {
    ...
    useEffect(() => {
        ...
        const loadSessions = async () => {
            try {
                ...
                const defaultActiveName = activeRes.data?.name || (serverSessions.find((s: any) => s.isDefault)?.name) || serverSessions[0]?.name;

                const currentSaved = localStorage.getItem('activeSession');
                const isValid = currentSaved && serverSessions.some((s: any) => s.name === currentSaved);

                if (!isValid && defaultActiveName) {
                    localStorage.setItem('activeSession', defaultActiveName);
                    setSessionState(defaultActiveName);
    ...
    return (
        <header className="topbar">
            ...
            <div className="topbar-right">
                ...
                {/* Divider */}
                <div className="topbar-divider" style={{ width: '1px', height: '32px', background: '#E5E7EB' }} />
                ...
                {/* User Profile */}
                <div className="user-profile" style={{ flexShrink: 0 }}>
                    <div style={{
                    ...
                    <div className="user-profile-text" style={{ textAlign: 'left' }}>
                        <p style={{ fontSize: '0.875rem', fontWeight: 700, margin: 0, color: '#1e293b', whiteSpace: 'nowrap' }}>
