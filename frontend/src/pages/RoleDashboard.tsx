    ...
    switch (role) {
        ...
        case 'TRANSPORT':
            return {
                ...
                subtitle: 'Routes, stops & student transport overview',
                accentColor: '#d69e2e',
                badge: { label: 'TRANSPORT', bg: '#fffff0', color: '#7b6a00' },
                stats: [
                    { title: 'Transport Stops', value: <AnimatedNumber value={statsData?.totalStops || 0} />, icon: <Bus size={22} />, color: '#d69e2e', trend: '' },
                    { title: 'Students Assigned', value: <AnimatedNumber value={statsData?.transporterCount || 0} />, icon: <Users size={22} />, color: '#4a90e2', trend: '' },
                    { title: 'Transport Collected', value: <AnimatedNumber value={statsData?.transportCollected || 0} isCurrency={true} />, icon: <Wallet size={22} />, color: '#48bb78', trend: '' },
                    { title: 'Transport Dues Pending', value: <AnimatedNumber value={statsData?.transportPending || 0} isCurrency={true} />, icon: <AlertCircle size={22} />, color: '#e53e3e', trend: '' },
                ],
            };
        case 'PARENT':
        case 'STUDENT':
...
const RoleDashboard: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const fetchDashboardData = async () => {
            try {
                ...
                if (['ADMIN', 'PRINCIPAL', 'ACCOUNTS'].includes(role)) {
                    ...
                    const [statsRes, stopsRes, dueRes] = await Promise.allSettled([
                        axios.get(`/erp-api/admin/dashboard/stats?session=${session}`),
                        axios.get('/erp-api/admin/transport/stops'),
                        axios.get(`/erp-api/fees/transport-due-list?session=${session}`)
                    ]);

                    const baseStats = statsRes.status === 'fulfilled' ? statsRes.value.data?.stats : {};
                    const stopsList = stopsRes.status === 'fulfilled' && Array.isArray(stopsRes.value.data) ? stopsRes.value.data : [];
                    const dueList = dueRes.status === 'fulfilled' && dueRes.value.data?.students ? dueRes.value.data.students : [];
                    
                    const transportCollected = dueList.reduce((sum: number, s: any) => sum + (s.totalPaid || 0), 0);
                    const transportPending = dueList.reduce((sum: number, s: any) => sum + (s.currentDue || 0), 0);
                    ...
                    if (statsRes.status === 'fulfilled' && statsRes.value.data?.recentActivities) {
                        setFetchedActivities(statsRes.value.data.recentActivities);
                    }
        ...
        window.addEventListener('activeSessionChanged', fetchDashboardData);
        return () => {
            clearInterval(clockTimer);
            window.removeEventListener('activeSessionChanged', fetchDashboardData);
        };
    }, [role, user.id]);
    ...
    const cssStyles = `
        ...
        .rd-header { display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 1.5rem; }
        .rd-quicklinks-grid { display: grid; gap: 1.25rem; }
        .rd-two-col { display: grid; grid-template-columns: 1.6fr 1fr; gap: 1.5rem; }
        .rd-two-col-admin { display: grid; grid-template-columns: 1.8fr 1fr; gap: 1.5rem; }
        .rd-three-col { display: grid; grid-template-columns: repeat(3, 1fr); gap: 1.5rem; align-items: stretch; }
        @media (max-width: 1100px) {
            .rd-two-col, .rd-two-col-admin { grid-template-columns: 1fr; }
            .rd-three-col { grid-template-columns: 1fr; }
        }
        @media (max-width: 768px) {
            .rd-header { padding: 1.75rem 1.5rem !important; text-align: left; }
            .rd-header > div:last-child { align-items: flex-start !important; text-align: left !important; }
            .rd-quicklinks-grid { grid-template-columns: repeat(auto-fit, minmax(110px, 1fr)) !important; }
        }
        @media (max-width: 480px) {
            .rd-quicklinks-grid { grid-template-columns: repeat(2, 1fr) !important; }
        }
    `;
    ...
    return (
        <div style={{ fontFamily: "'Inter', sans-serif", padding: '0.5rem 0', overflowX: 'hidden' }}>
            ...
            {/* ── Header ── */}
            <div className="animate-fade-in rd-header" style={{
            ...
            <div className="animate-fade-in delay-2" style={{ marginBottom: '2rem' }}>
                <SectionCard title="⚡ Quick Actions">
                    <div className="rd-quicklinks-grid" style={{
            ...
            {['ADMIN', 'PRINCIPAL', 'ACCOUNTS'].includes(role) && revenueData ? (
                <>
                    ...
                    <div className="animate-fade-in delay-4 rd-two-col" style={{ marginBottom: '2rem' }}>
                ...
                <div className={`animate-fade-in delay-3 ${role === 'ADMIN' ? 'rd-two-col-admin' : ''}`} style={{
                    display: role === 'ADMIN' ? undefined : 'block',
                    gap: '1.5rem',
                    marginBottom: '2rem'
                }}>
            ...
            <div className={`animate-fade-in delay-4 ${role === 'ADMIN' || role === 'PRINCIPAL' ? 'rd-three-col' : ''}`} style={{
                display: role === 'ADMIN' || role === 'PRINCIPAL' ? undefined : 'block',
                gap: '1.5rem',
                marginBottom: '2rem'
            }}>
                {/* ── Recent Activity ── */}
