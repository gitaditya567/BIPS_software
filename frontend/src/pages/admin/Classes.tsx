...
const Classes: React.FC = () => {
    ...
    const addClass = async (e: React.FormEvent) => {
        ...
        setLoading(true);
        try {
            await axios.post('http://localhost:5000/api/admin/classes', { name: newClassName });
            setNewClassName('');
            fetchClasses();
        } catch (err) {
            console.error(err);
    ...
    const addSection = async (e: React.FormEvent) => {
        ...
        setLoading(true);
        try {
            await axios.post(`http://localhost:5000/api/admin/classes/${selectedClassId}/sections`, { name: newSectionName });
            setNewSectionName('');
            setSelectedClassId('');
            fetchClasses();
        } catch (err) {
