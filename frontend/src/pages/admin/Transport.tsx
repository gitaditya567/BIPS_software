...
const Transport: React.FC = () => {
    ...
    const [newStop, setNewStop] = useState({ name: '', km: '', fee: '', ratePerKm: '' });
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.5fr', gap: '2.5rem', animation: 'fadeIn 0.4s ease-out' }}>
                            <div className="stat-card" style={{ display: 'block', height: 'fit-content', border: '1px solid #e0e7ff', background: '#f8faff', padding: '2rem' }}>
                                ...
                                <form onSubmit={handleAddTransportStop}>
                                    ...
                                    <div className="form-group">
                                        <label style={{ fontWeight: '700', color: '#475569' }}>Distance (KM)</label>
                                        <input 
                                            type="text" 
                                            className="form-control" 
                                            placeholder="e.g. 5.5" 
                                            value={newStop.km} 
                                            onChange={e => setNewStop({...newStop, km: e.target.value})} 
                                        />
                                    </div>
                                    <div className="form-group">
                                        <label style={{ fontWeight: '700', color: '#475569' }}>Rate (per KM)</label>
                                        <input 
                                            type="number" 
                                            className="form-control" 
                                            placeholder="e.g. 15" 
                                            value={newStop.ratePerKm} 
                                            onChange={e => {
                                                const rate = e.target.value;
                                                const kmVal = Number(newStop.km) || 0;
                                                const calculatedFee = (Number(rate) * kmVal).toString();
                                                setNewStop({...newStop, ratePerKm: rate, fee: calculatedFee});
                                            }} 
                                        />
                                    </div>
                                    <div className="form-group">
                                        ...
                                        <table style={{ width: '100%' }}>
                                            <thead>
                                                <tr style={{ backgroundColor: '#f8fafc' }}>
                                                    ...
                                                    <th style={{ padding: '1.25rem', color: '#64748b', fontSize: '0.85rem' }}>DISTANCE</th>
                                                    <th style={{ padding: '1.25rem', color: '#64748b', fontSize: '0.85rem' }}>RATE/KM</th>
                                            ...
                                            <tbody>
                                                {transportStops.length > 0 ? (
                                                    transportStops.slice().reverse().map((stop) => (
                                                        <tr key={stop.id} style={{ borderBottom: '1px solid #f1f5f9', transition: 'background 0.2s' }} onMouseEnter={e => e.currentTarget.style.backgroundColor = '#f8fafc'} onMouseLeave={e => e.currentTarget.style.backgroundColor = 'transparent'}>
                                                            ...
                                                            <td style={{ padding: '1.25rem', color: '#64748b' }}>
                                                                <span style={{ backgroundColor: '#f1f5f9', padding: '0.2rem 0.6rem', borderRadius: '6px', fontSize: '0.8rem', fontWeight: '600' }}>{stop.km} KM</span>
                                                            </td>
                                                            <td style={{ padding: '1.25rem', color: '#4f46e5', fontWeight: '600' }}>₹{stop.ratePerKm || 0}</td>
                                                    ...
                                                    <tr><td colSpan={5} style={{ padding: '4rem', textAlign: 'center', color: '#94a3b8' }}>No transport rates defined yet.</td></tr>
                                                )}
                                            </tbody>
                                        </table>
                                    </div>
